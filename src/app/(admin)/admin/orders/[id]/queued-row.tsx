...
import { type QueuedCommand, type Server } from "@/types";
...
export const QueuedRow = ({ command, servers, orderId }: { command: QueuedCommand, servers: Server[], orderId: string }) => {
  ...
  const deleteCommandFromQueue = api.orders.deleteCommandFromQueue.useMutation({
    onSuccess: async () => {
      await utils.orders.getQueuedCommands.invalidate({ id: orderId });
    },
  });
    ...
    <TableRow key={command.id}>
      ...
      <TableCell>
        <Button variant="destructive" className="w-fit" onClick={async () => {
          await deleteCommandFromQueue.mutateAsync({ id: command.id });
